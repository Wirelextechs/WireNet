...
export default function TelecelAdmin() {
  ...
  const loadOrders = async () => {
    try {
      console.log("📥 Fetching Telecel orders from /api/telecel/orders");
  ...
  const refreshAllOrderStatus = async () => {
    ...
    try {
      const response = await fetch("/api/telecel/orders/refresh/all", {
        method: "POST",
        credentials: "include",
      });
      if (response.ok) {
  ...
  const refreshSingleOrderStatus = async (orderId: string) => {
    try {
      const response = await fetch(`/api/telecel/orders/${orderId}/refresh`, {
        method: "POST",
        credentials: "include",
      });
      if (response.ok) {
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      const response = await fetch(`/api/telecel/orders/${orderId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
