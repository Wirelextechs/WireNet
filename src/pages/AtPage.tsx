...
export default function AtPage() {
  ...
  const [moolrePayerPhone, setMoolrePayerPhone] = useState("");
  const [moolrePayerNetwork, setMoolrePayerNetwork] = useState<"mtn" | "vodafone" | "airteltigo">("mtn");
  const [moolrePayerPhoneError, setMoolrePayerPhoneError] = useState("");
  ...
  const loadPackages = async () => {
    try {
      setLoading(true);
      const response = await fetch("/api/at/packages/public");
      if (response.ok) {
        const data = await response.json();
        setPackages(data);
      }
  ...
  const handleStatusCheck = async () => {
    ...
    try {
      const response = await fetch(`/api/at/orders/status/${statusCheckId}`);
      if (response.ok) {
  ...
  const addToCart = () => {
    ...
    if (!isValidPhone(phoneNumber)) {
      alert("Please enter a valid 10-digit AT number (e.g., 050xxxxxxx)");
      return;
    }
  ...
  const handlePaystackCheckout = async () => {
    ...
    try {
      const handler = (window as any).PaystackPop.setup({
        ...
        currency: "GHS",
        ref: `AT-BULK-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
        metadata: {
          wirenet: {
            service: "at",
            items: cart.map((item) => ({
        ...
        callback: function(response: any) {
          ...
          const processOrders = async () => {
            ...
            for (const item of cartItems) {
              try {
                const orderResponse = await fetch("/api/at/purchase", {
                  method: "POST",
            ...
            navigate(`/order/success/${orderId}?service=at`);
          };
  ...
  const handleMoolreCheckout = async () => {
    ...
    const charge = subtotal * (transactionCharge / 100);
    const totalAmount = subtotal + charge;
    const moolreRef = `AT-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    ...
    <div className="min-h-screen gradient-mesh">
      ...
      <AnimatePresence>
        {announcement.active && (
          <motion.div
            ...
            className="bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 text-white overflow-hidden"
          >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold flex items-center gap-2">
                ...
                <span className="text-gradient">AT iShare</span>
              </h1>
              <p className="text-sm text-muted-foreground">Instant delivery • Best rates</p>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 space-y-8">
        ...
        <motion.section
          ...
          className="rounded-3xl bg-gradient-to-br from-blue-500/10 via-indigo-500/10 to-violet-500/10 border border-blue-500/20 p-6"
        >
                ...
                <motion.div
                  ...
                  className={`
                    ...
                    ${selectedPackage?.id === pkg.id 
                      ...
                      : "bg-white/70 backdrop-blur-sm border border-white/30 hover:border-blue-300 hover:shadow-lg"}
                  `}
                >
                  {selectedPackage?.id === pkg.id && (
                    <motion.div
                  ...
                  <p className={`text-2xl font-bold ${selectedPackage?.id === pkg.id ? "text-white" : "text-blue-600"}`}>
                    {pkg.dataAmount}
                  </p>
        ...
        >
          ...
          <div className="p-5 rounded-2xl bg-white/70 backdrop-blur-sm border border-white/30">
            ...
            {selectedPackage ? (
              <div className="text-center">
                <p className="text-2xl font-bold text-blue-600">{selectedPackage.dataAmount}</p>
          ...
          <div className="p-5 rounded-2xl bg-white/70 backdrop-blur-sm border border-white/30 flex flex-col justify-center">
            <Button
              ...
              className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl gap-2"
            >
